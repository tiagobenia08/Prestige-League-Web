...
app.get("/api/rankings/players", async (_req, res) => {
  try {
    const result = await pool.query(`
      ...
      FROM players
      ORDER BY mmr DESC, COALESCE(NULLIF(psn, ''), NULLIF(discord_name, ''), discord_id) ASC
    `);
