...
async function getDiscordMemberCount() {
  ...
  const inviteCode = (process.env.DISCORD_INVITE_CODE || "bRDKns4TQ").replace(/^https?:\/\/(discord\.gg\/|discord\.com\/invite\/)/, "").replace(/\/$/, "");
  const botToken = process.env.DISCORD_TOKEN || process.env.DISCORD_BOT_TOKEN;

  // First use the official invite endpoint. It exposes Discord's approximate
  // member count without requiring the bot token and keeps the public site
  // synchronized with the actual server count.
  try {
  ...
  // Fallback: bot-authenticated guild endpoint.
  if (botToken) {
    try {
  ...
  // Final fallback: Discord server widget, if enabled.
  try {
